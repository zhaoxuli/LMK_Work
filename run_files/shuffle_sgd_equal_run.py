...
def run(wechat = False):
    ...
    models_path = './shuffle_sgd_equal_models'
    model_name = 'shuffle_sgd_equal_model'
    log_file = './shuffle_sgd_equal_train.log'
    learn_rate = 0.001
    ...
    sgd = ks.optimizers.SGD(lr=learn_rate, momentum=0.9, decay=0.0, nesterov=False)
    #adam =ks.optimizers.Adam(lr=0.001)
    model.compile(optimizer = sgd,loss = losses,loss_weights = lossWeights)#metrics = ["acc"])
